...
import { ArrowLeft, PlusCircle, Save, GripVertical, CheckCircle, Info, MessageSquare, ChevronDown } from 'lucide-react';
import {
  getStoryById,
  updateStory,
  createChapter,
...
export default function WebtoonDetailAdminPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (realStory && realStory.format === 'webtoon') {
        setStory(realStory);
        ...
        if (rawContributors.length > 0 && typeof rawContributors[0] === 'string') {
          setContributors((rawContributors as any[]).map(c => ({ role: 'Yazar', name: c })));
        } else if (rawContributors.length === 0) {
          // Default for new webtoons
          setContributors([{ role: 'Yazar', name: '' }, { role: 'Çizer', name: '' }]);
        } else {
          setContributors(rawContributors as Contributor[]);
        }
      } else {
        toast.error("Webtoon bulunamadı veya bu bir roman.");
        router.push('/studio/webtoons');
      }
    } catch (error) {
      console.error(error);
    } finally {
  ...
  }, [title, summary, coverImage, status, contributors, loading, story]);

  const handleAddChapter = async () => {
    try {
      const newChapId = await createChapter(storyId, {
        title: `Episod ${chapters.length + 1}`,
        order: chapters.length + 1,
        contentBlocks: [],
        status: 'draft' // Ensure it's created as draft
      });
      router.push(`/studio/webtoons/${storyId}/episodes/${newChapId}`);
    } catch (e) {
      toast.error("Episod oluşturulamadı.");
    }
  };

  const handleDeleteStory = async () => {
    const isConfirmed = window.confirm(
      "DİKKAT: Bu webtoonu, tüm episodlarını ve yorumlarını kalıcı olarak silmek üzeresiniz!\n\nBu işlemin hiçbir geri dönüşü YOKTUR. Silmek istediğinize emin misiniz?"
    );
    ...
    try {
      ...
      toast.success("Webtoon kalıcı olarak silindi.");
      router.push('/studio/webtoons');
    } catch (e) {
      console.error(e);
      toast.error("Webtoon silinirken bir hata oluştu.");
      setSaving(false);
    }
  };
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto w-full">
      <Button variant="ghost" onPress={() => router.push('/studio/webtoons')} className="mb-6 -ml-4">
        <ArrowLeft className="mr-2" /> Webtoonlarıma Dön
      </Button>

      <div className="flex justify-between items-start mb-8">
        <div>
          <Typography variant="h1">{title || 'İsimsiz Webtoon'}</Typography>
          <Typography variant="caption" className="text-muted mt-2">Webtoon detaylarını ve episodları buradan yönetebilirsiniz.</Typography>
        </div>
      ...
      <div className="mb-8 p-4 md:p-5 rounded-2xl bg-primary/10 border border-primary/20 flex flex-col md:flex-row gap-4 text-primary">
        ...
        <div className="text-sm">
          <strong className="block mb-2 text-base font-bold">Webtoon Stüdyosuna Hoş Geldiniz!</strong>
          <p className="mb-2 opacity-90">Bu sayfa webtoonunuzun ana merkezidir.</p>
          <ul className="list-disc ml-4 space-y-2 opacity-90">
            <li><strong className="font-semibold">Kapak ve Detaylar:</strong> Webtoon kapağını, özetini, yazar ve çizer kadrosunu ve yayın durumunu ayarlayın. (Otomatik kaydetme aktiftir.)</li>
            <li><strong className="font-semibold">Episodlar:</strong> "Yeni Ekle" butonuna basarak yeni bir episod oluşturun. Episod oluşturucuda yatay görsellerinizi dikey formatta parçalar halinde (slicelar) yükleyebilirsiniz.</li>
            <li><strong className="font-semibold">Episod Aktiviteleri:</strong> Okuyucularınızın episod sonlarındaki anket ve sorularınıza verdiği yanıtları "Episod Aktiviteleri" kısmından takip edebilirsiniz.</li>
          </ul>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-1 flex flex-col gap-6">
          <div className="bg-card p-6 rounded-2xl border border-border/20">
            <Typography variant="h3" className="mb-4">Webtoon Kapağı</Typography>
            <div className="w-full aspect-[2/3] bg-muted/10 rounded-xl overflow-hidden border-2 border-dashed border-border flex flex-col items-center justify-center relative mb-4">
              {coverImage ? (
                ...
                <Typography variant="caption" className="text-muted">Görsel Seç (480x720)</Typography>
              )}
              <input
                type="file"
                accept="image/*"
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <Typography variant="h3" className="font-bold text-amber-500">Episod Aktiviteleri</Typography>
            </div>
            <Typography variant="caption" className="text-muted">Episod sonlarına eklediğiniz anketlerin ve soruların cevaplarını görün.</Typography>
          </div>
          ...
          <div className="bg-red-500/5 p-6 rounded-2xl border border-red-500/20">
            ...
            <Typography variant="caption" className="text-muted block mb-4 leading-relaxed">
              Bu webtoonu ve altındaki tüm episodları kalıcı olarak silmek istiyorsanız aşağıdaki butonu kullanabilirsiniz. <strong>Bu işlemin hiçbir geri dönüşü yoktur.</strong>
            </Typography>
        ...
        <div className="md:col-span-2 flex flex-col gap-8">
          <div className="bg-card p-6 rounded-2xl border border-border/20 flex flex-col gap-4">
            <Typography variant="h3" className="mb-2">Webtoon Bilgileri</Typography>
            <Input
              value={title}
              onChangeText={setTitle}
              placeholder="Webtoon Adı"
            />
            <textarea
              value={summary}
              onChange={(e) => setSummary(e.target.value)}
              placeholder="Webtoon Özeti (Genel tanıtım)"
            ...
            <div className="flex flex-col gap-3 mt-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => setContributors([...contributors, { role: 'Yazar', name: '' }])}
                  ...
                  {contributors.map((contributor, idx) => (
                    <div key={idx} className="flex gap-3">
                      ...
                      >
                        ...
                        <option value="Renklendiren">Renklendiren</option>
          ...
          <div className="bg-card p-6 rounded-2xl border border-border/20">
            <div className="flex justify-between items-center mb-6">
              <Typography variant="h3">Episodlar</Typography>
                    ...
                    >
                      <div className="flex justify-between items-center">
                        ...
                        {chap.status === 'published' ? (
                           <span className="text-[10px] font-bold px-2 py-1 bg-green-500/10 text-green-500 rounded-full uppercase">Yayında</span>
                        ) : (
                           <span className="text-[10px] font-bold px-2 py-1 bg-muted/20 text-muted rounded-full uppercase">Taslak</span>
                        )}
                      </div>
                      <Typography variant="caption" className="text-muted mt-1">Sıra: {chap.order}</Typography>
                    </div>
                  </div>
                ))
              )}
